...
function MobileDaySched({teachers,students,onStudentClick}){
  ...
  const hourBlocks=useMemo(()=>{
    return TIMES.filter(t=>t.endsWith(":00")).map(hourTime=>{
      ...
      return{hourTime,h,s00,s30,hasAny:s00.length>0||s30.length>0};
    }).filter(b=>b.hasAny);
    ...
    <div>
      ...
      <div style={{display:"flex",gap:"4px",marginBottom:"1rem",overflowX:"auto",paddingBottom:"4px"}}>
        {DAYS.map(d=>{
          ...
          return(
            <button key={d} onClick={()=>setSelDay(d)} style={{flexShrink:0,border:`2px solid ${selDay===d?C.accent:C.border}`,borderRadius:"10px",padding:"6px 16px",cursor:"pointer",fontFamily:C.font,fontWeight:700,fontSize:"0.82rem",background:selDay===d?C.accent:"#fff",color:selDay===d?"#fff":has?C.text2:C.muted,transition:"all .15s"}}>{d}</button>
          );
        })}
      </div>
      {hourBlocks.length===0?(
        <div style={{textAlign:"center",padding:"2.5rem",color:C.muted,fontFamily:C.font,background:C.card2,borderRadius:"12px",border:`1px solid ${C.border}`}}>{selDay}요일 수업 없음</div>
      ):(
        <div>
          {hourBlocks.map(({hourTime,h,s00,s30})=>(
            <div key={hourTime} style={{marginBottom:"10px",background:C.card2,borderRadius:"12px",border:`1px solid ${C.border}`,overflow:"hidden"}}>
              <div style={{background:C.accent+"12",padding:"6px 14px",borderBottom:`1px solid ${C.border}`}}>
                <span style={{fontWeight:700,fontSize:"0.88rem",color:C.accent,fontFamily:C.font}}>{hourTime}</span>
              </div>
              <div style={{padding:"8px 12px"}}>
                {s00.map(s=>(
                  <div key={s.id} onClick={()=>onStudentClick(s)}
                    style={{background:s._teacher?s._teacher.color+"15":"#eff6ff",borderLeft:`3px solid ${s._teacher?.color||C.accent}`,borderRadius:"8px",padding:"8px 12px",marginBottom:"6px",cursor:"pointer",transition:"all .15s"}}
                    onMouseEnter={e=>e.currentTarget.style.opacity=".8"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
                    <span style={{fontWeight:700,color:s._teacher?.color||C.accent,fontFamily:C.font,fontSize:"0.9rem"}}>{s.name}</span>
                    <span style={{color:C.muted,marginLeft:"6px",fontSize:"0.75rem",fontFamily:C.font}}>{s.grade}</span>
                    {s._teacher&&<div style={{color:s._teacher.color,fontSize:"0.72rem",fontFamily:C.font,marginTop:"2px"}}>{s._teacher.name}</div>}
                  </div>
                ))}
                {s30.length>0&&(
                  <div>
                    <div style={{fontSize:"0.72rem",color:C.muted,fontFamily:C.font,fontWeight:600,marginBottom:"4px",marginTop:s00.length>0?"6px":0}}>{h}:30</div>
                    {s30.map(s=>(
                      <div key={s.id} onClick={()=>onStudentClick(s)}
                        style={{background:s._teacher?s._teacher.color+"15":"#eff6ff",borderLeft:`3px solid ${s._teacher?.color||C.accent}`,borderRadius:"8px",padding:"8px 12px",marginBottom:"6px",cursor:"pointer",transition:"all .15s"}}
                        onMouseEnter={e=>e.currentTarget.style.opacity=".8"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
...
/* ── 전체 시간표 (PC 테이블) ── */
function FullSchedPC({teachers,students,onStudentClick}){
                  ...
                  {showDays.map(d=>(
                    <td key={d} style={{padding:"2px 4px",verticalAlign:"top",minWidth:"88px"}}>
                      {isHour&&byday[d].map(s=>(
                        <div key={s.id} onClick={()=>onStudentClick(s)}
                          style={{background:s._teacher?s._teacher.color+"15":"#eff6ff",borderLeft:`3px solid ${s._teacher?.color||C.accent}`,borderRadius:"6px",padding:"3px 7px",marginBottom:"3px",fontSize:"0.72rem",color:C.text,fontFamily:C.font,lineHeight:1.5,cursor:"pointer",transition:"all .15s"}}
...
/* ── 선생님별 시간표 (모바일) ── */
function TeacherSchedMobile({teacher,students,onStudentClick}){
  const [selDay,setSelDay]=useState(DAYS[0]);
  ...
  const hourBlocks=useMemo(()=>{
    return TIMES.filter(t=>t.endsWith(":00")).map(hourTime=>{
      const h=hourTime.split(":")[0];
      const s00=getSlots(selDay,hourTime);
      const s30=getSlots(selDay,`${h}:30`);
      return{hourTime,h,s00,s30,hasAny:s00.length>0||s30.length>0};
    }).filter(b=>b.hasAny);
  },[selDay,students,teacher]);
    ...
    <div>
      ...
      <div style={{display:"flex",gap:"4px",marginBottom:"1rem",overflowX:"auto",paddingBottom:"4px"}}>
        {DAYS.map(d=>{
          ...
          return<button key={d} onClick={()=>setSelDay(d)} style={{flexShrink:0,border:`2px solid ${selDay===d?teacher.color:C.border}`,borderRadius:"10px",padding:"6px 14px",cursor:"pointer",fontFamily:C.font,fontWeight:700,fontSize:"0.82rem",background:selDay===d?teacher.color+"22":"#fff",color:selDay===d?teacher.color:has?C.text2:C.muted,transition:"all .15s"}}>{d}</button>;
        })}
      </div>
      {hourBlocks.length===0?(
        <div style={{textAlign:"center",padding:"2rem",color:C.muted,fontFamily:C.font,background:C.card2,borderRadius:"12px",border:`1px solid ${C.border}`}}>{selDay}요일 수업 없음</div>
      ):(
        <div>
          {hourBlocks.map(({hourTime,h,s00,s30})=>(
            <div key={hourTime} style={{marginBottom:"10px",background:C.card2,borderRadius:"12px",border:`1px solid ${C.border}`,overflow:"hidden"}}>
              <div style={{background:teacher.color+"12",padding:"6px 14px",borderBottom:`1px solid ${C.border}`}}>
                <span style={{fontWeight:700,fontSize:"0.88rem",color:teacher.color,fontFamily:C.font}}>{hourTime}</span>
              </div>
              <div style={{padding:"8px 12px"}}>
                {s00.map(s=>(
                  <div key={s.id} onClick={()=>onStudentClick(s)}
                    style={{background:teacher.color+"15",borderLeft:`3px solid ${teacher.color}`,borderRadius:"8px",padding:"8px 12px",marginBottom:"6px",cursor:"pointer"}}
                    onMouseEnter={e=>e.currentTarget.style.opacity=".8"} onMouseLeave={e=>e.currentTarget.style.opacity="1"}>
...
/* ── 선생님별 시간표 (PC 테이블) ── */
function TeacherSchedPC({teacher,students,onStudentClick}){
...
function ReportModal({students,onUpdateRecord}){
  ...
  const filteredTargets=useMemo(()=>{
    ...
    if(reportSearch.trim()){const q=reportSearch.toLowerCase();base=base.filter(s=>s.name?.includes(q)||s.school?.toLowerCase().includes(q)||s.grade?.includes(q));}
    return base;
  },[selSid,sortedStudents,reportSearch]);

  function doDetail(){const[y,m]=selYM.split("-");const rows=[];filteredTargets.forEach(s=>dates.forEach(date=>{const r=s.records?.[date]||{};if(r.att||r.hw||r.memo)rows.push([s.name,s.grade,s.school,date,ATT_LABEL[r.att]||"",r.hw?.toUpperCase()||"",r.memo||""]);})
  );exportCSV(`출결_숙제_상세_${y}년${m}월`,["이름","학년","학교","날짜","출결","숙제","메모"],rows);}
  function doSummary(){const[y,m]=selYM.split("-");const rows=filteredTargets.map(s=>{const recs=dates.map(d=>s.records?.[d]||{});return[s.name,s.grade,s.school,recs.filter(r=>r.att==="present").length,recs.filter(r=>r.att==="absent").length,recs.filter(r=>r.att==="makeup").length,...HW_OPTS.map(h=>recs.filter(r=>r.hw===h).length)];});exportCSV(`출결_숙제_요약_${y}년${m}월`,["이름","학년","학교","출석","결석","보강","A","B","C","D","N"],rows);}
    ...
    <div>
      ...
      <div style={{display:"flex",gap:"8px",marginBottom:"1rem",flexWrap:"wrap",alignItems:"center"}}>
        ...
        <select value={selSid} onChange={e=>{setSelSid(e.target.value);setReportSearch("");}} style={{background:C.card2,border:`1.5px solid ${C.border}`,borderRadius:"8px",padding:"7px 10px",color:C.text,fontSize:"0.875rem",outline:"none",fontFamily:C.font}}>
          <option value="all">전체 원생</option>{sortedStudents.map(s=><option key={s.id} value={s.id}>{s.name} ({s.grade})</option>)}
        </select>
      ...
      {selSid==="all"&&<input value={reportSearch} onChange={e=>setReportSearch(e.target.value)} placeholder="🔍 이름·학교·학년 검색" style={{width:"100%",background:C.card2,border:`1.5px solid ${C.border}`,borderRadius:"8px",padding:"8px 14px",color:C.text,fontSize:"0.875rem",outline:"none",fontFamily:C.font,boxSizing:"border-box",marginBottom:"0.75rem"}} onFocus={e=>e.target.style.borderColor=C.accent} onBlur={e=>e.target.style.borderColor=C.border}/>}
...
export default function App(){
  ...
  const filtered=useMemo(()=>{
    ...
    return sortedStudents.filter(s=>(!q||s.name?.includes(q)||s.school?.toLowerCase().includes(q)||s.grade?.includes(q))&&(fT==="all"||s.teacherId===fT)&&(fSub==="all"||s.subject===fSub));
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:C.font}}>
        ...
        {isMobile?(
          <div>
            ...
            <div style={{display:"flex",borderTop:`1px solid ${C.border}`}}>
              {TABS.map(t=>(
                <button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,border:"none",padding:"9px 4px",cursor:"pointer",fontFamily:C.font,fontWeight:600,fontSize:"0.78rem",background:tab===t.id?"#eff6ff":C.card,color:tab===t.id?C.accent:C.text2,borderBottom:tab===t.id?`2px solid ${C.accent}`:"2px solid transparent",transition:"all .15s"}}>{t.l}</button>
              ))}
            </div>
          </div>
        ):(
      ...
      <main style={{maxWidth:"1300px",margin:"0 auto",padding:isMobile?"0.75rem":"1.25rem"}}>
        ...
        {tab==="schedule"&&(
          <div style={{background:C.card,borderRadius:"14px",border:`1px solid ${C.border}`}}>
            <div style={{padding:isMobile?"0.75rem 1rem":"1rem 1.25rem",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"8px"}}>
            ...
            <div style={{padding:isMobile?"0.75rem 1rem":"1rem 1.25rem"}}>
              {schedView==="full"&&(
                isMobile
                  ?<MobileDaySched teachers={teachers} students={students} onStudentClick={handleStudentClick}/>
                  :<FullSchedPC teachers={teachers} students={students} onStudentClick={handleStudentClick}/>
              )}
              {schedView==="byTeacher"&&(
                <div>
                  ...
                  {tvId?(
                    isMobile
                      ?<TeacherSchedMobile teacher={teachers.find(x=>x.id===tvId)} students={students} onStudentClick={handleStudentClick}/>
                      :<TeacherSchedPC teacher={teachers.find(x=>x.id===tvId)} students={students} onStudentClick={handleStudentClick}/>
            ...
            {isMobile?(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                {filtered.map(st=>{
                  ...
                  return(
                    <div key={st.id} onClick={()=>setSelS(st)}
                      style={{background:C.card,borderRadius:"12px",padding:"0.85rem 1rem",border:`1px solid ${C.border}`,cursor:"pointer",display:"flex",alignItems:"center",gap:"12px",position:"relative",overflow:"hidden"}}>
                      ...
                      <span style={{color:C.muted,fontSize:"1.1rem"}}>›</span>
                    </div>
                  );
                })}
      ...
      {selT&&!editT&&(<Modal title={`${selT.name} 시간표`} onClose={()=>setSelT(null)} wide zIndex={200}><div style={{display:"flex",gap:"6px",marginBottom:"1rem"}}><Btn sm onClick={()=>setEditT(selT)}>✏️ 정보 수정</Btn><Btn sm v="danger" onClick={()=>delTeacher(selT.id)}>🗑️ 삭제</Btn></div>
        {isMobile?<TeacherSchedMobile teacher={selT} students={students} onStudentClick={handleStudentClick}/>:<TeacherSchedPC teacher={selT} students={students} onStudentClick={handleStudentClick}/>}
      </Modal>)}
      ...
      {schedStudentData&&(<Modal title={`${schedStudentData.name} 원생 상세`} onClose={()=>setScheduleStudentModal(null)} wide zIndex={300}><StudentDetail student={schedStudentData} teacher={teachers.find(t=>t.id===schedStudentData.teacherId)} onEdit={()=>{setEditS(schedStudentData);setScheduleStudentModal(null);}} onDelete={()=>delStudent(schedStudentData.id)} onUpdateRecord={updateRecord}/></Modal>)}
